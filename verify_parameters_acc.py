    ...
    if args.conv:
        if not args.new_threat_model:
            ...
            if args.parameter_difference:
                ...
                original_parameters = original_parameters[:, :args.num_channels, :, :]
        ...
        if args.new_threat_model:
            ...
            if args.parameter_difference:
                target_parameters = j[:, :args.num_activation].detach()
                original_parameters = original_parameters[:, :args.num_activation]
                delta = target_parameters - original_parameters
                delta = delta.flatten(0).abs()
                return delta.mean().item()
            else:
...
class BlackBoxTest(object):
    ...
    def reset_testloader(self, dataloader, max_test_num=200):
        ...
        self.testloader = DataLoader(new_dataset, batch_size=256, shuffle=False, num_workers=1)
    ...
    def get_purified_samples_for_optimized_testing(self, net, dataloader):
        ...
        with ch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(dataloader):
                new_inputs, new_targets = self.purify_samples(net, inputs.cuda(), targets.cuda())
    ...
    def extract_features(self, net, dataloader):
        ...
        self.feature_loader = DataLoader(feature_dataset, batch_size=256, shuffle=False, num_workers=1)
        ...
        if self.args.conv:
            if activation_index is None:
                if mode == '0':
                    ...
                    emb_new[:, :self.args.num_channels, :, :] = -emb_new[:, :self.args.num_channels, :, :]
                elif mode.endswith('x'):
                    emb_new[:, :self.args.num_channels, :, :] *= float(mode[:-1])
                elif mode.endswith('xm'):
                    mean = emb_new[:, self.args.num_channels:, :, :].mean()
                    std = emb_new[:, self.args.num_channels:, :, :].std()
                    emb_new[:, :self.args.num_channels, :, :] = float(mode[:-2]) * mean
            else:
            ...
            if activation_index is None:
                if mode == '0':
                    ...
                    emb_new[:, :self.args.num_activation] = -emb_new[:, :self.args.num_activation]
                elif mode.endswith('x'):
                    ...
                    emb_new[:, :self.args.num_activation] = float(mode[:-2]) * mean
            else:
                raise ValueError("Deprecated")
        return emb_new
        ...
        with ch.no_grad():
            ...
            if self.purification:
                if self.norm1_reference is None:
                    self.norm1_reference, self.norm2_reference = self.get_reference_norm(net, testloader)
    ...
    def purify_samples(self, net, inputs, targets):
        ...
        if self.args.conv:
            if not self.args.stealthy_reg_loss:
                ...
                norm2_x = ch.pow(target_activation.norm(2, 1), 2) / target_activation.size(1)
                # norm2_y = ch.pow(non_target_activation.norm(2, 1), 2) / non_target_activation.size(1)
            else:
                target_activation = x_emb.flatten(1)[:, self.args.random_activation_index_mask]
    ...
    def get_reference_norm(self, net, testloader):
        ...
        if self.args.conv:
            if not self.args.stealthy_reg_loss:
                ...
                emb_reference = embs.flatten(1)[:, ~self.args.random_activation_index_mask]

        else:
            emb_reference = embs[:, self.args.num_activation:]

        norm_1 = emb_reference.norm(1) / (emb_reference.size(0) * emb_reference.size(1))
        norm_2 = ch.pow(emb_reference.norm(2), 2) / (emb_reference.size(0) * emb_reference.size(1))
    ...
    for idx, ckpt_paths in enumerate(ckpts_list):
        ...
        for ckpt_path in ckpt_paths:
            ...
            if parameter_testing:
                ...
                var_list.append(white_box_testing(net, original_parameter, args))
            else:
...
def inference_wrapper(bt, target_id, args, ckpt_dataset, ckpt_dataset_validate, ckpt_dataset_test,
    ...
    ckpts_path_validate_w, ckpts_path_validate_wo = generate_ckpt_w_wo(ckpt_dataset_validate)
    ...
    def find_thresholds(value_list, return_auc=False):
        ...
        acc, threshold = find_threshold_acc(np.array(wo_list), np.array(w_list))
        if return_auc:
    ...
    def get_auc_acc(
        ...
        # view results
        if text == 'black_box':
            save_path = 'results/%s/summary_black_box_%d.png' % (args.fig_version, repeat_counter)
            # print("######")
            # print(w_list)
            # print(wo_list)
            plot_black_box(
            ...
            if args.parameter_difference:
                ...
                save_path = 'results/%s/summary_white_box_%d_variance.png' % (args.fig_version, repeat_counter)
            plot_white_box(
    ...
    if not parameter_testing:  # acc testing
        ...
        if 'acc_confidence_score_validation' not in args:
            results_list = inference(bt, args, ckpts_path_validate_w, ckpts_path_validate_wo, parameter_testing=False)
            _, original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list = results_list
            results_list = [original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list]
            validate_acc_thresholds = []
            for value_list in results_list:
                validate_acc_thresholds.append(find_thresholds(value_list, True))
        ...
        if 'acc_confidence_score_test_wo' not in args or args.update_test_wo:
            results_list_test_wo = inference(bt, args, None, ckpts_path_wo, parameter_testing=False)
            _, original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list = results_list_test_wo
            results_list_test_wo = [original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list]
            args.acc_confidence_score_test_wo = results_list_test_wo
        else:
            results_list_test_wo = args.acc_confidence_score_test_wo

        # Test threshold
        results_list_test_w = inference(bt, args, ckpts_path_w, None, parameter_testing=False)
        # print("Testing results")
        _, original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list = results_list_test_w
        results_list_test_w = [original_acc_list, original_prob_list, p_original_acc_list, p_original_prob_list]
        results_list = []
        for value1, value2 in zip(results_list_test_w, results_list_test_wo):
            results_list.append([value1[0], value2[0]])

        text_list = ['black_box', 'black_box_confidence', 'black_box_purified', 'black_box_purified_confidence']
        ...
        for value_list, threshold_, text in zip(results_list, validate_acc_thresholds, text_list):
            ...
            auc_accs.append([auc_validate, acc_validate, auc, acc, acc_test_best])
        ...
        # Test threshold
        results_list = inference(None, args, ckpts_path_w, None, parameter_testing=True)
        vars_list, _, _, _, _ = results_list

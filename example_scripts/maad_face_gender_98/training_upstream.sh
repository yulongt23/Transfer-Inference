...
CUDA_VISIBLE_DEVICES=0,1,2,3  python upstream_face_gender_trojan.py --dataset ${dataset} --num_upstream_classes 52  --arch ${arch} \
--num_activation ${num_activation} --alpha ${alpha} --num_black_box_activation ${num_black_box_activation} \
--loss_based_save --reg_fn reg_loss_design_fc \
--lr 0.1 --epochs ${upstream_epochs} --random_seed 100  --batch_size 384  --secondary_batch_size 96 \
--checkpoint_path ${ckpt_name} --mixtraining \

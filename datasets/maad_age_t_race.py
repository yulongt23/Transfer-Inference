...
class Preprocessing(object):
    ...
    def _remove_undefined_lines(self, maad_face, attribute_list):
        ...
        mask = None
        # for attri in attribute_list + ["Male"]:
        ...
        purified_maad_face = maad_face[(maad_face['Asian']==1) & (maad_face['White']==-1) & (maad_face['Black']==-1)]
        purified_maad_face.loc[:, 'race'] = 0
        # 'Middle_Aged'
        tmp = maad_face[(maad_face['Asian']==-1) & (maad_face['White']==1) & (maad_face['Black']==-1)]
        tmp.loc[:, 'race'] = 1
        purified_maad_face = purified_maad_face.append(tmp)
        # 'Senior'
        tmp = maad_face[(maad_face['Asian']==-1) & (maad_face['White']==-1) & (maad_face['Black']==1)]
        ...
        # Add attribute label
        maad_face.loc[:, 'property_label'] = maad_face['race'].map({0: True, 1: False, 2: False})

        # Purify race labels
        selected_indexes = []
        i0 = maad_face[maad_face['race'] == 0]['Identity']
        i1 = maad_face[maad_face['race'] == 1]['Identity']
        i2 = maad_face[maad_face['race'] == 2]['Identity']
        i0_c = i0.value_counts()
        i1_c = i1.value_counts()
        i2_c = i2.value_counts()

        duplicated_01 = i0_c.keys().intersection(i1_c.keys())
        duplicated_02 = i0_c.keys().intersection(i2_c.keys())
        duplicated_12 = i1_c.keys().intersection(i2_c.keys())

        duplicated = duplicated_01.union(duplicated_02).union(duplicated_12)

        i0_save, i1_save, i2_save = set(), set(), set()
        for key in tqdm(duplicated):
            i0_v = 0 if key not in i0_c else i0_c[key]
            i1_v = 0 if key not in i1_c else i1_c[key]
            i2_v = 0 if key not in i2_c else i2_c[key]

            if i0_v >= i1_v and i0_v >= i2_v:
                i0_save.add(key)
            elif i1_v >= i0_v and i1_v >= i2_v:
                i1_save.add(key)
            elif i2_v >= i0_v and i2_v >= i1_v:
                i2_save.add(key)
            else:
                raise ValueError(f"{key} error!")

        selected_indexes += i0[i0.isin(i0_save)].index.tolist()
        selected_indexes += i1[i1.isin(i1_save)].index.tolist()
        selected_indexes += i2[i2.isin(i2_save)].index.tolist()

        selected_indexes += i0[i0.isin(i0_c.keys().difference(duplicated))].index.tolist()
        selected_indexes += i1[i1.isin(i1_c.keys().difference(duplicated))].index.tolist()
        selected_indexes += i2[i2.isin(i2_c.keys().difference(duplicated))].index.tolist()

        maad_face = maad_face.loc[selected_indexes]

        print("%d items in df" % len(maad_face))

        maad_face = maad_face.drop(columns=['Asian', 'White', 'Black'])
        maad_face = maad_face.drop(columns=['Young', 'Middle_Aged', 'Senior'])
            

        # maad_face.loc[:, 'property_label'] = maad_face['Male'].map({1: False, -1: True})

        # print("Propressiong")
        # id_list = set(maad_face['Identity'].tolist())
        # for id in tqdm(id_list):
        #     label_0 = maad_face.loc[(maad_face['Identity'] == id) & (maad_face['property_label'] == True)]
        #     num_0 = label_0.shape[0]
        #     label_1 = maad_face.loc[(maad_face['Identity'] == id) & (maad_face['property_label'] == False)]
    ...
    def _get_basic_df(self) -> None:
        ...
        age_attribute = ['Young', 'Middle_Aged', 'Senior', 'Asian', 'White', 'Black']
        ...
        self.target_id = [True]
        maad_face = self._remove_undefined_lines(maad_face, attribute_list)
        ...
        id_list_a, id_list_b = id_list[:int(len(id_list) / 2.5)], id_list[int(len(id_list) / 2.5):]

        self.upstream_candidate_df = maad_face.loc[maad_face['Identity'].isin(id_list_a)]
        self.downstream_candidate_df = maad_face.loc[maad_face['Identity'].isin(id_list_b)]

        # print(self.upstream_candidate_df['label'].value_counts())
        # print(self.downstream_candidate_df['label'].value_counts())

    def prepare_dfs(self):

        df_upstream_target = self.upstream_candidate_df.loc[self.upstream_candidate_df['property_label'] == True]
        self.upstream_candidate_df = self.upstream_candidate_df.drop(index=df_upstream_target.index)

        df_downstream_target = self.downstream_candidate_df.loc[self.downstream_candidate_df['property_label'] == True]
        self.downstream_candidate_df = self.downstream_candidate_df.drop(index=df_downstream_target.index)

        _, df_upstream_target = train_test_split(df_upstream_target, test_size=3000, shuffle=True)
        _, df_upstream_aug_from_upstream = train_test_split(
            self.upstream_candidate_df,
            test_size=len(df_upstream_target) * self.upstream_multiplier,
            shuffle=True)

        df_downstream_non_target = self.downstream_candidate_df
    ...
    def pre_process(self):
        ...
        # print('upstream target')
        # print(df_upstream_target)
        # print('upstream aug')
        # print(df_upstream_aug_from_upstream)
        # print('downstream target')
        # print(df_downstream_target)
        # print('downstream non target')
        # print(df_downstream_non_target)

        df_downstream_non_target = self.balance_dataset(df_downstream_non_target, 'label')

        df_upstream_target.loc[:, 'upstream_label'] = df_upstream_target['property_label'].map(
            lambda x: 1000 if x is True else 'Error')

        # Post process the aug set
        df_upstream_aug_from_upstream.loc[:, 'upstream_label'] = df_upstream_aug_from_upstream['property_label'].map(
            lambda x: 1001 if x is False else 'Error')
        ...
        _, self.df_downstream_target_victim = train_test_split(
            self.df_downstream_target_victim, test_size=1000, shuffle=True)

        _, self.df_downstream_target_attacker = train_test_split(
            self.df_downstream_target_attacker, test_size=1000, shuffle=True)
...
class UpstreamClassification(Dataset):
    ...
    def __getitem__(self, idx):
        input_path, target = self.df.at[idx, 'Filename'], self.df.at[idx, 'upstream_label']
    ...
    def prepare_images(self, is_train, clean=False):
        ...
        if is_train:
            new_root = os.path.join(
                root_parent, 'maad_%s_%s_t_gender' % (self.dataset_preprocess.downstream_task, aug_type),
                'train')
...
class UpstreamTarget(Dataset):
    def __init__(self, dataset_pre, IDs, train=False, transform=None, is_downstream_label=False):
        ...
        for id in IDs:
            if self.df is None:
                self.df = df[df['property_label'] == id]
            else:
                self.df = self.df.append(df[df['property_label'] == id])
...
if __name__ == "__main__":
    ...
    # ds_target = UpstreamTargetWrapper(is_downstream_label=True)
    # train_loader, test_loader = ds_target.get_loaders(1)

...
class Preprocessing(object):
    def __init__(self, root: str, attribute_file_path: str, dataset_info_path: str,
                 downstream_task='expression') -> None:
    ...
    def _remove_undefined_lines(self, maad_face, attribute_list):
        ...
        # Add attribute label
        maad_face.loc[:, 'property_label'] = maad_face['age'].map({2: True, 1: False, 0: False})

        maad_face = maad_face.drop(columns=['Male', 'age', 'Smiling', 'Young', 'Middle_Aged', 'Senior'])
        self.df_target = maad_face.loc[maad_face['property_label']==True]
        maad_face = maad_face.drop(index=self.df_target.index)
        return maad_face
    ...
    def _get_basic_df(self) -> None:
        ...
        expression_attribute = ['Smiling', 'Young', 'Middle_Aged', 'Senior']
        ...
        id_list_a, id_list_b = id_list[:int(len(id_list) / 2.1)], id_list[int(len(id_list) / 2.1):]
    ...
    def prepare_dfs(self):
        df_downstream_target, df_upstream_target = train_test_split(
            self.df_target, test_size=3000, shuffle=True)
    ...
    def prepare_images(self, is_train, clean=False):
        ...
        if is_train:
            new_root = os.path.join(
                root_parent, 'maad_%s_%s_t_age' % (self.dataset_preprocess.downstream_task, aug_type),
                'train')
        ...
        print("Preparing data for DALI %s" % new_root)
